...
say "\nNot koha-plugin-edifact-libraria, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-libraria";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-libraria';
    next if $repo eq 'koha-plugin-edifact-libraria-docs';
